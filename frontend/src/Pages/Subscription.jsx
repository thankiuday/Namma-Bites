  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-gradient-to-br from-orange-50 to-white min-h-screen rounded-2xl shadow-2xl border border-orange-100">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 mb-10 border border-orange-100">
        ...
        {success && <div className="text-green-600 mt-4 font-semibold text-center">{success}</div>}
        <button type="submit" className="mt-6 px-8 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl hover:from-orange-600 hover:to-orange-700 font-bold shadow-lg transition-all duration-200">
          {loading ? 'Saving...' : 'Create Plan'}
        </button>
      </form>

      <h2 className="text-2xl font-extrabold mb-4 text-orange-700 drop-shadow">Your Subscription Plans</h2>
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-10 border border-orange-100">
        {plans.length === 0 ? (
          <div className="text-gray-700">No plans found.</div>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-orange-100 bg-orange-50">
            <table className="min-w-full text-sm">
              <thead className="bg-orange-100">
                ...
                {plans.map(plan => (
                  <tr key={plan._id} className="even:bg-orange-50">
                    ...
                    <td className="px-3 py-2 text-gray-900 flex gap-2">
                      <button onClick={() => startEdit(plan)} className="text-blue-600 hover:text-blue-800 flex items-center gap-1 mr-2 px-3 py-1 rounded-lg border border-blue-200 bg-blue-50 hover:bg-blue-100 transition-all duration-150"><FaEdit /> Edit</button>
                      <button
        ...
        {deleteError && <div className="text-red-600 mt-4 font-semibold text-center">{deleteError}</div>}
      </div>
      ...
      {editingPlan && editForm && (
        <form onSubmit={handleEditSubmit} className="bg-white rounded-2xl shadow-lg p-6 mb-10 border border-orange-100">
          <h3 className="text-xl font-extrabold mb-4 text-orange-700 drop-shadow">Edit Subscription Plan</h3>
          <div className="flex flex-col md:flex-row gap-6 mb-6">
            <div className="flex-1">
              <label className="block font-semibold mb-2 text-gray-700">Duration</label>
          ...
          {editError && <div className="text-red-600 mt-4 font-semibold text-center">{editError}</div>}
          {editSuccess && <div className="text-green-600 mt-4 font-semibold text-center">{editSuccess}</div>}
